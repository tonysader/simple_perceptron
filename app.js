...
request.onupgradeneeded = function(event) {
    ...
    if (!db.objectStoreNames.contains(tablename)) { // if there's no "books" store
...
.controller('MyFirstController',['$scope', function ($scope)  {
  ...
  $scope.startTesting = function(){
    ...
    for(let i=0;i<$scope.testing.values.length;i++){
        $scope.testing.sum += $scope.testing.values[i] * $scope.network.inputs[i].weight;
    }
    $scope.testing.output =($scope.testing.sum >= $scope.network.bias) ? 1 : 0;
  ...
  $scope.start_train= function(){
    ...
    while(!solved){
      ...
      $scope.network.trainings.forEach((tr, i) => {
        ...
        tr.output = 0;
        for(let inIdx = 0;inIdx< $scope.network.inputs_num; inIdx++){
          eq += "+" + tr.output + " + " +tr.values[inIdx] + "*"  + $scope.network.inputs[inIdx].weight ;

          tr.output += tr.values[inIdx] * $scope.network.inputs[inIdx].weight;
          eq += "= " + tr.output + ";";
        }
        tr.afterBias = (tr.output >= $scope.network.bias)? 1:0;
        eq += tr.afterBias == 1 ? "Output = 1;" : "Output=0;";
        if(tr.afterBias == tr.desired){
          eq += "Do not modify weights because output is like the desired value ;";

          tr.solved = true;
        }else{
          eq += "Calculating new weights: ;";
          for(let inIdx = 0;inIdx< $scope.network.inputs_num; inIdx++){
            $scope.network.inputs[inIdx].weight = $scope.network.inputs[inIdx].weight +  $scope.network.c *(tr.desired - tr.output) * tr.values[inIdx];
            eq+= "new weight number " + (inIdx + 1) + "="+  $scope.network.inputs[inIdx].weight +"= "+  $scope.network.inputs[inIdx].weight + " + " + $scope.network.c + "*(" + tr.desired +"-"+ tr.output + ")*" + tr.values[inIdx] + "=" + $scope.network.inputs[inIdx].weight +";";
          }
        }
      });
      let subSolved = true;
  ...
  $scope.change_trainings_num= function(){
    for(let i=0; i<$scope.network.trainings_num;i++){
      $scope.network.trainings.push({name: "s" + (i+1) , values: [],desired: 0,output: 0,solved : false,afterBias: 0 });
  ...
  function readAll() {
   ...
   request.onsuccess= function(){
     ...
     $scope.oldNetworks.forEach((item, idx) => {
       $("#all_old_networks").append('<li class="mt-3 old_net_li"><button type="button" class="btn btn-dark text-white old_network_choice" network_idx="'+idx+'"  name="button">'+ item.name + ' </button></li>');
